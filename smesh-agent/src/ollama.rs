...
use crate::backend::{
    GenerateRequest as BackendRequest, GenerateResponse as BackendResponse,
...
impl OllamaClient {
    ...
    pub async fn generate(&self, prompt: &str, system: Option<&str>) -> Result<String, OllamaError> {
        ...
        if !resp.status().is_success() {
            ...
            return Err(OllamaError::GenerationFailed(
...
impl LlmBackend for OllamaClient {
    ...
    async fn generate(&self, request: BackendRequest) -> Result<BackendResponse, LlmError> {
        ...
        let result = OllamaClient::generate(self, &request.prompt, request.system.as_deref()).await
