...
use thiserror::Error;
use quinn::{Endpoint, ServerConfig, ClientConfig, Connection, RecvStream};
...
impl QuicTransport {
    ...
    pub async fn send(&self, addr: SocketAddr, msg: TransportMessage) -> Result<(), TransportError> {
        ...
        let connection = match connection {
            ...
            None => {
                ...
                let conns = self.connections.read().await;
                conns.get(&addr).cloned()
                    .ok_or_else(|| TransportError::ConnectionFailed("Connection not established".into()))?
            }
        };
    ...
    pub async fn broadcast(&self, addrs: &[SocketAddr], signal: Signal) -> Vec<Result<(), TransportError>> {
        let mut results = Vec::new();
        
        for addr in addrs {
            let result = self.send(*addr, TransportMessage::Signal(signal.clone())).await;
            results.push(result);
        }
