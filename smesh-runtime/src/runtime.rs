...
use smesh_core::{Network, Signal, Node};
use crate::peer::{PeerManager, PeerId};
...
impl SmeshRuntime {
    ...
    pub async fn emit(&self, signal: Signal, node_id: &str) -> Option<String> {
        ...
        let _ = self.event_tx.send(RuntimeEvent::SignalEmitted { 
            hash: hash.clone() 
        }).await;
    ...
    pub async fn run(&self) {
        ...
        info!("SMESH runtime starting with {}ms tick interval", 
        ...
        loop {
            ...
            match &event {
                RuntimeEvent::TickCompleted { tick, active_signals, expired } => {
                    if *tick % 100 == 0 {
                        debug!(
...
mod tests {
    use super::*;
    use smesh_core::{SignalType, NetworkTopology};
