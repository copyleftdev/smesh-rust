...
use crate::{Signal, DEFAULT_TRUST, MIN_TRUST, MAX_TRUST};
...
impl Node {
    ...
    pub fn get_trust(&self, other_node_id: &str) -> f64 {
        *self.trust_scores.get(other_node_id).unwrap_or(&DEFAULT_TRUST)
    }
    ...
    pub fn update_trust(&mut self, other_node_id: &str, delta: f64) -> f64 {
        ...
        self.trust_scores.insert(other_node_id.to_string(), new_trust);
        new_trust
    }
    ...
    pub fn should_escalate(&self, signals: &[Signal]) -> bool {
        ...
        let max_reinforcements = signals.iter().map(|s| s.reinforcement_count).max().unwrap_or(0);
